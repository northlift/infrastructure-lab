...
4. **Cross-project protocols**: See global AGENTS.md (`~/.pi/agent/AGENTS.md`) for:
   - Failure Recovery Protocol (3 strikes rule)
   - Clarification Protocol (question format)
   - Task Completion (Ralph Loop)
   - Second Brain / knowledge capture
5. **Fish shell**: The operator uses fish, not bash. Use fish-compatible syntax in all commands.
...
4. **Match lessons to knowledge base**: If the lesson applies beyond this project (cross-project), create or update a learning entry in `~/knowledge/learnings/`. Project pitfalls and skills are derivative copies; the knowledge base is the source of truth.
